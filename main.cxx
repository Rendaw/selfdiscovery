...
int main(int argc, char **argv)
{
	try {
		...
		if (FindProgramArgument("help").first) RunMode = RunModes::Help;
		...
		{
			std::cout << "\tA controller is a program that passes requests for information to this program and processes the returned information.  The controller is run as a subprocess of this program, and may be any generally recognized program type, such as an executable or shell script.\n"
				"\tEach request may be one or more lines of space-separated values, followed by a blank line.  The responses are one or more lines of information followed by a blank line.  The controller should only exit before making a request, not during this program's response.\n"
		...
		if ((RunMode == RunModes::Help) || (RunMode == RunModes::ControllerHelp)) ResponseStream.Write("help");
